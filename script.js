...
//audio volume ranges from 0-1 so target value divided by 100 will give a value between 0 & 1 
...
Array.from(document.getElementsByClassName("card")).forEach(card => {
    card.addEventListener("click", () => {
        let title = card.querySelector(".trending-songs-title").innerText.trim();

        // Find matching song from songs array
        let matchedSong = songs.find(song =>
            song.replaceAll("%20", " ").includes(title)
        );
